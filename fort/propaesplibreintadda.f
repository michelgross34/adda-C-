...
!DEC$ ATTRIBUTES DLLEXPORT :: propaespacelibreintadda
        subroutine propaespacelibreintadda(Rij, k0a, gridspacex, 
     $   gridspacey, gridspacez, relreq, result, IFAIL) 
     $  bind(C, name="propaespacelibreintadda_")

       use iso_c_binding
        implicit none

c     ================== ARGUMENTS (INTERFACE C) ==================

       real(C_DOUBLE), intent(in)  :: Rij(3)
       real(C_DOUBLE), intent(in)  :: k0a
      real(C_DOUBLE), intent(in)  :: gridspacex, gridspacey, gridspacez
       real(C_DOUBLE), intent(in)  :: relreq
        real(C_DOUBLE), intent(out) :: result(12)
...
c     ,wavelength, spacing lattice
      ...
      double precision gridspacexm, gridspaceym, gridspacezm
...
c     Variables needed for the integration

      integer KEY, N, NF, NDIM, MINCLS, MAXCLS, NEVAL, NW
       parameter (NW=4000000, NDIM=3, NF=12)

        double precision A(NDIM), B(NDIM), WRKSTR(NW)
       double precision ABSEST(NF), ABSREQ, RELREQ_LOC, err
