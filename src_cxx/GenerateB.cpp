...
extern int beam_Npars;
extern double beam_pars[];
...
#ifndef NO_FORTRAN
extern "C" void bjndd_(const int *n,const double *x,double *bj,double *dj,double *fj);
#endif
...
// initialize beam; produce description string
{
	...
	const char *tmp_str=NULL; // temporary string
	/* TO ADD NEW BEAM
...
// generates incident beam at every voxel center
{
	...
	double phi,arg,td1[100],td2[100],jn1[100]; // for Bessel beams
#endif
	const char *fname;
	/* TO ADD NEW BEAM
...
//				tmp=cDotProd_Im(v2,v1);
		...
		case B_BES_TML:
			...
			for (i=0;i<local_nvoid_Ndip;i++) {
				...
				arg=creal(besKt)*sqrt(x*x+y*y); // argument of Bessel functions
				if (arg<ROUND_ERR) {
