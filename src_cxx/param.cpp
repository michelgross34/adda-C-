...
extern bool volcor_used;
extern const char *sh_form_str1,*sh_form_str2;
#ifndef SPARSE
extern size_t gr_N;
extern double gr_vf_real;
#endif //SPARSE
extern size_t mat_count[];
...
PARSE_FUNC(m)
{
	...
	for (i=0;i<Nmat;i++) {
		...
		ref_index[i] = mre + I*mim;
		 if ( creal( ref_index[i])==1  &&  cimag( ref_index[i])==0) PrintErrorHelp("Given refractive index #%d is that of vacuum, which is not supported. "
