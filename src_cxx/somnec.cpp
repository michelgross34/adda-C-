...
static void bessel(double__complex z,double__complex *j0,double__complex *j0p);
static void gshank(double__complex start,double__complex dela,double__complex *sum,int nans,double__complex *seed,int ibk,
	double__complex bk,double__complex delb);
static void hankel(double__complex z,double__complex *h0,double__complex *h0p);
static void lambda(double t,double__complex *xlam,double__complex *dxlam);
static void rom1(int n,double__complex *sum,int nx);
static void saoa(double t,double__complex *ans);
...
static double__complex ct1,ct2,ct3,ck1,ck1sq,cksm;
static double__complex xl0; // location of surface-plasmon pole
...
// common /cntour/
static double__complex a,b;

//======================================================================================================================

// a single function from cmplx.h not to include the whole file
static inline double cAbs2(const double__complex z)
...
// evaluates the zero-order Bessel function and its derivative for complex argument z
{
	...
	double zms;
	double__complex p0z,p1z,q0z,q1z,zi,zi2,zk,cz,sz,j0x=0,j0px=0;
 ...
 */
{
	...
	static double__complex cp1,cp2,cp3,bk,delta,delta2,sum[6],ans[6];
	...
	if (!jump) { // this is always enabled when mode==2 (or any other unrecognized value)
		...
		if (denser) { // here the SP pole is already accounted for by shift of point c
			...
			if (pole) { // this can be optimized by singularity extraction
				double__complex rot=(1+I*EPS_TAN)/(1+EPS_TAN*EPS_TAN); // rotation multiplier for EPS_TAN
				double__complex dP3=xl0-cp3;
 ...
 */
{
	...
	static double__complex a1,a2,as1,as2,del,aa;
	static double__complex q1[6][MAXH],q2[6][MAXH],ans1[6],ans2[6];
...
// evaluates the Hankel function of the first kind, order zero, and its derivative for complex argument z
{
	...
	double__complex clogz,j0,j0p,p0z,p1z,q0z,q1z,y0=0,y0p=0,zi,zi2,zk;
...
// integrates the 6 Sommerfeld integrals from a to b in lambda. Variable-interval-width Romberg integration is used
{
	...
	static double__complex t00,t11,t02;
	static double__complex g1[6],g2[6],g3[6],g4[6],g5[6],t01[6],t10[6],t20[6];
...
{
	double xlr;
	static double__complex xl,dxl,cgam1,cgam2,b0,b0p,com,dgam,den1,den2;
